...
def open_seller_page():
   ...
   #---------------label for entry frame ---------------------------
    ...
    submit_button = tk.Button(
        ...
        command=lambda: submit_details(description_entry.get(), price_entry.get())
    )
    submit_button.grid(row=40, columnspan=2, pady=60)  # Centered below the entry fields

def submit_details(description, base_price):
    try:
        db = connect_to_db()
        cursor = db.cursor()

        # Define the parameters for the stored procedure
        user_id = 1  # Replace with the actual User_ID or retrieve dynamically
