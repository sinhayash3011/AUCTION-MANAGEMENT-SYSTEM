...
import preview  # Assumes a separate preview.py file for the preview window functionality
...
# Function to submit bid to the database, updating both Bid and Bidder_Bid tables
# Function to submit bid to the database, allowing multiple bids on the same item by the same user
def submit_bid(item_id, bid_amount, bidder_id):
    try:
        ...
        if bid_amount >= base_price:
            # Insert into the Bid table and retrieve the new Bid_ID
            cursor.callproc('submit_bid', [item_id, bid_amount, 'Not Won'])
            connection.commit()

            # Get the newly inserted Bid_ID
            cursor.execute("SELECT LAST_INSERT_ID()")
            bid_id = cursor.fetchone()[0]

            # Insert the bid information into Bidder_Bid table
            cursor.execute("INSERT INTO Bidder_Bid (Bidder_ID, Bid_ID) VALUES (%s, %s)", (bidder_id, bid_id))
            connection.commit()

            # Update the status of bids for this item
...
def open_bid_entry_popup(item_id, base_price, bidder_id):
    def submit_bid_action():
        ...
        if bid_amount >= base_price:
            submit_bid(item_id, bid_amount, bidder_id)  # Pass bidder_id
            bid_popup.destroy()
        else:
            messagebox.showwarning("Warning", f"Bid amount must be at least ${base_price:.2f}")
...
# Function to handle Apply for Bid button click, passing bidder_id to the pop-up
# Function to handle Apply for Bid button click, checking if payment is made before bidding
...
def preview_bid():
    ...
    if selected_bid_index:
        ...
        # Send the bid details to preview.py
        preview.open_preview_window(selected_bid)
    else:
...
def open_bidder_page(bidder_id):
    global item_table, applied_bids_table
